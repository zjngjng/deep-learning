...
def discriminator1(images, reuse=False):
    """
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        x1 = tf.layers.conv2d(resized_images, 128, 5, strides=2, padding='same')
        ...
        x3 = tf.layers.conv2d(x1, 512, 5, strides=2, padding='same')
...
def discriminator(images, reuse=False):
    ...
    # TODO: Implement Function
    with tf.variable_scope('discriminator', reuse=reuse):
        x1 = tf.layers.dense(images, 1024)
        x1 = tf.maximum(0.2*x1, x1)

        x2 = tf.layers.dense(x1, 512)
        x2 = tf.maximum(0.2*x2, x2)

        x3 = tf.layers.dense(x2, 256)
        x3 = tf.maximum(0.2 * x3, x3)
...
def generator1(z, out_channel_dim, is_train=True):
    """
...
def generator(z, out_channel_dim, is_train=True):
    ...
    # TODO: Implement Function
    with tf.variable_scope('generator', reuse=not is_train):
        x1 = tf.layers.dense(z, 256)
        x1 = tf.maximum(0.2 * x1, x1)

        x2 = tf.layers.dense(x1, 512)
        x2 = tf.maximum(0.2 * x2, x2)

        x3 = tf.layers.dense(x2, 1024)
        x3 = tf.maximum(0.2 * x3, x3)

        logits = tf.layers.dense(x3, 784)
        out = tf.tanh(logits)
